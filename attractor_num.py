...
def main():
    result = {}
    if not os.path.exists("attractor_num.json"):
                ...
                for const in range(0, 2):
                    ...
                    for sim_no in range(simu_num):
                        with open(f"Save/{node_num}node_{degree}degree_{const}const_0.5prob/attractor{sim_no}.json", "r") as f:
                            ...
                            result[f"{node_num}node_{degree}degree_{const}const"].append(len(data["attractors"]))

        with open("attractor_num.json", "w") as f:
            json.dump(result, f)
    with open("attractor_num.json", "r") as f:
        result = json.load(f)
